...
function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!session && isProtected && current!== '(auth)/login') {
      router.replace('/(auth)/login' as never); return;
    }
    if (session && !profile && !inAuth && current !== 'create-profile') {
      router.replace('/(auth)/create-profile' as never); return;
    }
    if (session && profile && !termsOk && !inTerms && !inAuth) {
      router.replace('/terms' as never); return;
    }
    if (session && termsOk) {
      if (profile?.estado === 'pendiente' &&!inPending) router.replace('/pending' as never);
      else if ((profile?.estado === 'rechazado' || profile?.estado === 'suspendida') &&!inDenied) router.replace('/denied' as never);
      else if (profile?.estado === 'activa' && (inAuth || inTerms || inPending || inDenied)) router.replace('/(tabs)' as never);
      else if (!profile?.estado && isProtected) router.replace('/(auth)/create-profile' as never);
    }
