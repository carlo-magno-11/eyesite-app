...
export default function MyPropertiesScreen() {
  ...
  useEffect(()=>{ if(!user)return; let alive=true; (async()=>{const {data,error}=await supabase.from("propiedades").select("id,codigo,titulo,tipo,municipio,precio_actual,estado,activa,portada_url,created_at").eq("user_id",user.id).order("created_at",{ascending:false}); if(alive){setItems(data||[]);setLoading(false);} if(error)console.error("[my-properties]",error);})(); return()=>{alive=false};},[user?.id]);
