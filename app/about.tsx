          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Cuenta eliminada', 'Tu cuenta y los datos asociados fueron eliminados.', [
                { text: 'Aceptar', onPress: () => router.replace('/(auth)/login' as never) },
              ]);
            } catch (error) {
              console.error('[account-delete]', error);
              Alert.alert('No se pudo eliminar', 'No pudimos completar la eliminación. Inténtalo de nuevo o contacta a EYESITE.');
            }
          },
        },
      ]
    ...
    <ScreenContainer edges={['top', 'left', 'right']} containerClassName="bg-background">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View style={styles.accountSection}>
          <Text style={styles.sectionTitle}>CUENTA Y PRIVACIDAD</Text>
          <Pressable onPress={() => router.push('/privacy' as never)} style={styles.accountRow}>
            <Text style={styles.accountRowText}>Aviso de Privacidad</Text>
            <Text style={styles.accountRowArrow}>›</Text>
          </Pressable>
          <Pressable onPress={handleDeleteAccount} style={[styles.accountRow, styles.deleteRow]}>
            <Text style={styles.deleteText}>Eliminar mi cuenta</Text>
            <Text style={styles.accountRowArrow}>›</Text>
          </Pressable>
          <Text style={styles.accountHint}>Puedes solicitar la eliminación de tu cuenta directamente desde la aplicación.</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  accountSection: { paddingHorizontal: 20, paddingBottom: 24 },
  accountRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#1A1A1A', borderWidth: 1, borderColor: '#2A2A2A', borderRadius: 10, padding: 16, marginBottom: 8 },
  deleteRow: { borderColor: '#4A2929' },
  accountRowText: { color: '#F5F5F5', fontSize: 14, fontWeight: '700' },
  deleteText: { color: '#E57373', fontSize: 14, fontWeight: '700' },
  accountRowArrow: { color: '#C9A84C', fontSize: 24 },
  accountHint: { color: '#777', fontSize: 11, lineHeight: 17, marginTop: 4 },
  footer: {
    alignItems: 'center',
    paddingVertical: 24,
    borderTopWidth: 1,
