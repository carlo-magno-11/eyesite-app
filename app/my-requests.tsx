...
export default function MyRequestsScreen() {
  ...
  useEffect(()=>{ if(!user)return; let alive=true; (async()=>{const {data,error}=await supabase.from("solicitudes_propiedades").select("id,titulo,tipo,municipio,precio_actual,estado,motivo_rechazo,created_at,updated_at,propiedad_id").eq("user_id",user.id).order("created_at",{ascending:false}); if(alive){setItems(data||[]);setLoading(false);} if(error)console.error("[my-requests]",error);})(); return()=>{alive=false};},[user?.id]);
