  ...
  return (
    <ScreenContainer edges={["top","left","right"]} containerClassName="bg-background">
      <ScrollView contentContainerStyle={[s.page,{paddingHorizontal:horizontalPadding,maxWidth:contentMaxWidth,width:"100%",alignSelf:"center"}]} showsVerticalScrollIndicator={false}>
        ...
        {loading?<ActivityIndicator color="#C9A84C"/>:items.length===0?<Text style={s.empty}>No tienes solicitudes todavía.</Text>:
          items.map(p=><View key={p.id} style={s.card}>
            ...
            {p.motivo_rechazo?<Text style={s.reason}>Motivo: {p.motivo_rechazo}</Text>:null}
            {p.propiedad_id&&p.estado==="aprobada"?<Pressable onPress={()=>router.push(("/property/"+p.propiedad_id) as never)}><Text style={s.link}>VER PUBLICACIÓN ›</Text></Pressable>:null}
          </View>)
        }
      </ScrollView>
    </ScreenContainer>
