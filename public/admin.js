...
function confirmar(
  ...
  const cov =
    document.getElementById('cov');

  const ctit =
    document.getElementById('ctit');

  const csub =
    document.getElementById('csub');

  const cok =
    document.getElementById('cok');
...
const sectionInfo = {
  ...
  propiedades: {
    title: 'Propiedades',
    subtitle:
      'Administra las propiedades de la plataforma'
  },
...
async function cargarDashboard() {
  ...
  const stip =
    document.getElementById('stip');

  const savg =
    document.getElementById('savg');
...
async function renderPendientes() {
  ...
  const tbody =
    document.getElementById('pendtb');
...
function propertyFormHTML(p={},mode='new'){
 const tipo=String(p.tipo||p.type||'terreno').toLowerCase(); const d=p.detalles&&typeof p.detalles==='object'?p.detalles:{};
 const house=['casa','hacienda','rancho','departamento','villa'].some(x=>tipo.includes(x))||Number(p.construccion_m2||0)>0||d.recamaras!=null||d.banos!=null;
 const v=k=>p[k]??'';
...
function readNumberField(id) { const v=valueOf(id); if(v==='') return null; const n=Number(v); return Number.isFinite(n)?n:null; }
function readJsonField(id) { const raw=valueOf(id); if(!raw) return {}; try{return JSON.parse(raw);}catch(_){return {texto:raw};} }
function collectPropertyForm(mode,statusOverride=null){
 const estado=statusOverride||valueOf(`${mode}_estado`)||(mode==='new'?'activa':'activa');
 const tipo=valueOf(`${mode}_tipo`)||'terreno';
 const detalles=(mode==='edit'&&propiedadEditando?.detalles&&typeof propiedadEditando.detalles==='object')?{...propiedadEditando.detalles}:{};
 for(const k of ['recamaras','banos','estacionamientos','plantas','ano_construccion']){const v=valueOf(`${mode}_${k}`);if(v!=='')detalles[k]=Number(v);}
 const data={titulo:valueOf(`${mode}_titulo`),tipo,municipio:valueOf(`${mode}_municipio`),direccion:valueOf(`${mode}_direccion`)||null,superficie:readNumberField(`${mode}_superficie`)??0,unidad_superficie:valueOf(`${mode}_unidad_superficie`)||'m2',frente:readNumberField(`${mode}_frente`),fondo:readNumberField(`${mode}_fondo`),precio_actual:readNumberField(`${mode}_precio`)??0,precio_mercado:readNumberField(`${mode}_precio_mercado`),precio_esperado:readNumberField(`${mode}_precio_esperado`),unidad_precio:valueOf(`${mode}_unidad_precio`)||'m2',moneda:valueOf(`${mode}_moneda`)||'MXN',rendimiento:readNumberField(`${mode}_rendimiento`)??0,estatus_legal:valueOf(`${mode}_estatus_legal`)||'Sin revisar',certeza_legal:!!document.getElementById(`${mode}_certeza_legal`)?.checked,destacada:!!document.getElementById(`${mode}_destacada`)?.checked,descripcion:valueOf(`${mode}_descripcion`)||null,descripcion_pro:valueOf(`${mode}_descripcion_pro`)||null,caracteristicas:readJsonField(`${mode}_caracteristicas`),servicios_cercanos:readJsonField(`${mode}_servicios_cercanos`),contacto_nombre:valueOf(`${mode}_contacto_nombre`)||null,contacto_telefono:valueOf(`${mode}_contacto_telefono`)||null,contacto_whatsapp:valueOf(`${mode}_contacto_whatsapp`)||null,contacto_email:valueOf(`${mode}_contacto_email`)||null,dueno_nombre:valueOf(`${mode}_dueno_nombre`)||null,dueno_telefono:valueOf(`${mode}_dueno_telefono`)||null,dueno_email:valueOf(`${mode}_dueno_email`)||null,tour_360:valueOf(`${mode}_tour_360`)||null,paquete:valueOf(`${mode}_paquete`)||'basico',precio_sesion:readNumberField(`${mode}_precio_sesion`)??0,sesion_pagada:!!document.getElementById(`${mode}_sesion_pagada`)?.checked,sesion_fecha:valueOf(`${mode}_sesion_fecha`)||null,comision_porcentaje:readNumberField(`${mode}_comision_porcentaje`)??5,detalles,estado:estado==='aprobado'||estado==='activo'?'activa':estado==='rechazado'?'inactiva':estado,activa:estado!=='inactiva'&&estado!=='rechazado',status:estado==='inactiva'?'inactiva':'activa'};
 data.construccion_m2=readNumberField(`${mode}_construccion_m2`); return data;
}
...
function validateProperty(data) {
  ...
  if (
    ...
    Number.isNaN(data.precio_actual)
  ) {
    toast(
      'Indica el precio.'
    );
  ...
  confirmar(
    ...
    async () => {
      ...
      try {
        ...
        const payload = {
          ...data,
          fotos: images.map(item => typeof item === 'string' ? item : item?.url).filter(Boolean),
          imagenes:
            images,
...
async function saveEdit() {
  if (!propiedadEditando) {
    toast(
      'No hay una propiedad seleccionada.'
    );
...
async function cambiarEstadoPendiente(id, estado) {
  try {
    if (estado === 'aprobado') {
      const solicitud = pendientes.find(item => String(item.id) === String(id));
      ...
      console.info('[admin] solicitud aprobada, propiedad:', data);
    } else {
      const motivo = prompt('Motivo del rechazo:');
      if (!motivo || !motivo.trim()) { toast('Debes indicar el motivo del rechazo.'); return; }
      const { error } = await s.rpc('admin_reject_property_request', { p_request_id: id, p_reason: motivo.trim() });
      if (error) throw error;
    }
    toast(estado === 'aprobado' ? 'Propiedad aprobada y publicada.' : 'Solicitud rechazada.');
    closeMod('vmod'); pendienteViendo = null;
    await cargarPendientes(); await cargarPropiedades(); await cargarDashboard();
    renderPendientes(); renderPropiedades();
  } catch (error) {
    console.error('[admin moderation]', error); toast(error?.message || 'No se pudo procesar la solicitud.');
  }
}
/* ============================================================
   RECHAZAR
                ...
                ${
                  estado === 'aprobado'
                    ? `
                      <button
                        class="bs brj2"
                        onclick="bloquearUsuario('${esc(usuario.id)}')"
                      >
                        🔒 Bloquear
                      </button>
                    `
...
async function aprobarUsuario(id) {
  ...
  confirmar(
    '¿Aprobar usuario?',
    `${usuario.email || 'Este usuario'} podrá continuar con su cuenta.`,
    async () => {
      try {
        let result =
          await s
...
async function bloquearUsuario(id) {
  ...
  confirmar(
    '¿Bloquear usuario?',
    `${usuario.email || 'Este usuario'} quedará en revisión.`,
    async () => {
      try {
        let result =
          await s
...
/* ============================================================
   NOTIFICACIONES
   ============================================================ */
async function cargarNotificacionesAdmin(){const {data,error}=await s.from('notificaciones').select('*').order('created_at',{ascending:false}).limit(50);if(error)return;const b=document.getElementById('ntb');if(b)b.innerHTML=(data||[]).map(n=>`<tr><td>${dateTimeMX(n.created_at)}</td><td class="p">${esc(n.titulo)}</td><td>${esc(n.tipo||'info')}</td><td>${n.user_id?'Usuario':'TODOS'}</td><td>${esc(n.mensaje)}</td></tr>`).join('')||'<tr><td colspan="5">No hay notificaciones.</td></tr>';}
async function crearNotificacionAdmin(){const titulo=valueOf('nt_titulo'),mensaje=valueOf('nt_mensaje'),tipo=valueOf('nt_tipo')||'info',destino=valueOf('nt_destino')||'all',userId=valueOf('nt_user');if(!titulo||!mensaje){toast('Título y mensaje son obligatorios.');return;}if(destino==='one'&&!userId){toast('Indica el UUID del usuario.');return;}try{let ids=[];if(destino==='all'){const {data,error}=await s.from(TABLE_PROFILES).select('id').eq('estado','activa').neq('role','admin');if(error)throw error;ids=(data||[]).map(u=>u.id);}else ids=[userId];if(ids.length){const {error}=await s.from('notificaciones').insert(ids.map(id=>({user_id:id,titulo,mensaje,tipo,leida:false})));if(error)throw error;}try{await s.functions.invoke('send-notification',{body:{titulo,mensaje,tipo,user_id:destino==='one'?userId:null}});}catch(e){console.warn('[push]',e);}toast('Notificación enviada correctamente.');document.getElementById('nt_titulo').value='';document.getElementById('nt_mensaje').value='';await cargarNotificacionesAdmin();}catch(e){console.error(e);toast(e?.message||'No se pudo enviar la notificación.');}}
function setupNotifications(){const d=document.getElementById('nt_destino'),w=document.getElementById('nt_user_wrap');if(d&&w)d.addEventListener('change',()=>w.style.display=d.value==='one'?'block':'none');cargarNotificacionesAdmin();s.channel('admin-live-notifications').on('postgres_changes',{event:'*',schema:'public',table:'notificaciones'},cargarNotificacionesAdmin).subscribe();}
...
function setupModalEvents() {
  ...
  const emod =
    document.getElementById('emod');

  if (emod) {
    emod.addEventListener(
      'click',
      event => {
        if (
          event.target === emod
  ...
  const vmod =
    document.getElementById('vmod');

  if (vmod) {
    vmod.addEventListener(
      'click',
      event => {
        if (
          event.target === vmod
...
async function initAdmin() {
  try {
    ...
    setupNotifications();
    s.channel('admin-live-properties').on('postgres_changes',{event:'*',schema:'public',table:TABLE_PROPERTIES},async()=>{await cargarPropiedades();await cargarDashboard();renderPropiedades();}).subscribe();
    s.channel('admin-live-submissions').on('postgres_changes',{event:'*',schema:'public',table:TABLE_SUBMISSIONS},async()=>{await cargarPendientes();await cargarDashboard();}).subscribe();
    ...
    console.log(
      'EYESITE Admin iniciado correctamente.'
    );
